...
import static jdplus.dfm.base.core.DfmMapping.max;
...
public class DfmMapping2 implements IDfmMapping {
    ...
    public DfmMapping2(DynamicFactorModel model, final boolean mfixed, final boolean tfixed) {
        ...
        nl = template.getVarDescriptor().getNlags();
    ...
    public IMultivariateSsf map(DoubleSeq p) {
        ...
        if (l != null) {
            ...
            for (MeasurementDescriptor desc : template.getMeasurements()) {
                ...
                double x = mv.get(j0++);
                builder.measurement(dbuilder.coefficient(DoubleSeq.of(c))
                        .variance(x * x)
                        .build());
                ++n;
            }
        ...
        if (vp != null) {
            @NonNull TransitionDescriptor var = template.getVar();
